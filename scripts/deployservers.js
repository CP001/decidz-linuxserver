			...
			if(jsondata.hasOwnProperty(key)) {
				...
				if (fileexists(FLAGFILE_FOLDER + "deploy_" + key + ".flag")) {
					...
					if (servertype == "nodewebsite") {
						...
						console.log("Deploying website - " + description);
						logstuff(glogfilename, "Deploying website - <b>" + description + "</b>");
						deploysimplewebsite(glogfilename, key, description);
					} else if (servertype == "pfbtoazure") {
						console.log("Deploying website - " + description);
						deploypfbtoazure(glogfilename, key, description);						
					} else {
				...
				} else if (fileexists(FLAGFILE_FOLDER + "stop_" + key + ".flag")) {
					var fs = require('fs');
					var fbkey = dateFormat(now, "yyyymmdd-HHMMss-") + Guid.raw();
					var glogfilename = fbkey + ".html";
					console.log("Request to stop " + key + " node script");
					logstuff(glogfilename, FLAGFILE_FOLDER + "stop_" + key + ".flag detected");
					logstuff(glogfilename, "renaming file to " + FLAGFILE_FOLDER + "stop_" + key + ".processing");
					movefile(FLAGFILE_FOLDER + "stop_" + key + ".flag", FLAGFILE_FOLDER + "stop_" + key + ".processing");
					console.log("Killing script " + key + '_worker.js');
					logstuff(glogfilename, "Killing script " + key + '_worker.js');
//					if ((key.toLowerCase().indexOf("_staging") > -1) || (key.toLowerCase().indexOf("_int") > -1)) {
						output = execSync(HELPERS_SCRIPTS_FOLDER_LOCATION + 'killpm2nodescript.sh ' + key);
//					} else {
//						output = execSync(HELPERS_SCRIPTS_FOLDER_LOCATION + 'killnodescript.sh ' + key + '_worker.js');
//					}
					logstextarea(glogfilename, output, 3);
					console.log("script killed");
					logstuff(glogfilename, "Script killed");
					try {
						description = jsondata[key].description;
					}
					catch (e) {
					...
					deployment_email = "";
					try {
						var jsondeploymentdata = JSON.parse(fs.readFileSync(FLAGFILE_FOLDER + "stop_" + key + ".processing", 'utf8'));
						deployment_email = jsondeploymentdata.useremail;
					} catch (err) {
						console.log(err);
					}
					deletefile(FLAGFILE_FOLDER + "stop_" + key + ".processing");
					if (fileexists(FLAGFILE_FOLDER + "start_" + key + ".flag")) {
						deletefile(FLAGFILE_FOLDER + "start_" + key + ".flag");
					}
					messageslackchannel("Node script " + description + " stopped (requested by " + deployment_email + ") - <" + WEBSITE_LOGS_LOCATION + glogfilename + "|view log file>");
				} else if (fileexists(FLAGFILE_FOLDER + "start_" + key + ".flag")) {
					var fs = require('fs');
					var fbkey = dateFormat(now, "yyyymmdd-HHMMss-") + Guid.raw();
					var glogfilename = fbkey + ".html";
					node_website_root = jsondata[key].node_website_root;
					console.log("Request to start " + key + " node script");
					logstuff(glogfilename, FLAGFILE_FOLDER + "start_" + key + ".flag detected");
					logstuff(glogfilename, "renaming file to " + FLAGFILE_FOLDER + "start_" + key + ".processing");
					movefile(FLAGFILE_FOLDER + "start_" + key + ".flag", FLAGFILE_FOLDER + "start_" + key + ".processing");
					console.log("Starting script " + key + '_worker.js');
					logstuff(glogfilename, "Starting script " + key + '_worker.js');
//					if ((key.toLowerCase().indexOf("_staging") > -1) || (key.toLowerCase().indexOf("_int") > -1)) {
						output = execSync(HELPERS_SCRIPTS_FOLDER_LOCATION + 'startpm2nodescript.sh ' + node_website_root + '/' + key + '_worker.js ' + key + ' ' + node_website_root);
//					} else {
//						output = execSync(HELPERS_SCRIPTS_FOLDER_LOCATION + 'startnodescript.sh ' + node_website_root + ' ' + key + '_worker.js');
//					}
					logstextarea(glogfilename, output, 3);
					console.log("script started");
					logstuff(glogfilename, "Script started");
					try {
						description = jsondata[key].description;
					}
					catch (e) {
					...
					deployment_email = "";
					try {
						var jsondeploymentdata = JSON.parse(fs.readFileSync(FLAGFILE_FOLDER + "start_" + key + ".processing", 'utf8'));
						deployment_email = jsondeploymentdata.useremail;
					} catch (err) {
						console.log(err);
					}
					deletefile(FLAGFILE_FOLDER + "start_" + key + ".processing");
					messageslackchannel("Node script " + description + " started (requested by " + deployment_email + ") - <" + WEBSITE_LOGS_LOCATION + glogfilename + "|view log file>");
				}
			}
		}		
	}
...
function deploypfbtoazure(glogfilename, sitename, description) {
	var fs = require('fs');
...
//	logstextarea(glogfilename, outputtext, 5);
	...
	// copy folder to remote azure server
	console.log("Copying prefirebase to azure server");
	logstuff(glogfilename, "Copying prefirebase to azure server");
	var outputtext = sh.exec(HELPERS_SCRIPTS_FOLDER_LOCATION + 'copypfbtoazure.sh', {silent:true}).output;
	...
	messageslackchannel("Deployment to <http://decidz2.cloudapp.net/|" + description + "> complete (requested by " + deployment_email + ") - <" + WEBSITE_LOGS_LOCATION + glogfilename + "|view log file>");
}

function deploysimplewebsite(glogfilename, sitename, description) {
	var fs = require('fs');
	console.log("Loading details for site " + description);
	logstuff(glogfilename, "Loading details for site " + description);
	messageslackchannel("Deployment to " + description + " started - <" + WEBSITE_LOGS_LOCATION + glogfilename + "|view log file>");
	...
	if (jsondata != "") {
		...
		if((description != "") && (description)){
			...
			deployment_nginx_host_name = jsondata[sitename].nginx_host_name;
			...
			deployment_source_folder = jsondata[sitename].source_folder;
			console.log("Copying folder " + gitrepo_local_folder + deployment_source_folder + " (" + deployment_branch + ") to " + deployment_node_website_root);
			logstuff(glogfilename, "Copying folder " + gitrepo_local_folder + deployment_source_folder + " (" + deployment_branch + ") to " + deployment_node_website_root);
			var outputtext = sh.exec(HELPERS_SCRIPTS_FOLDER_LOCATION + 'copyfolder.sh ' + gitrepo_local_folder + deployment_source_folder + " " + deployment_node_website_root + " " + deployment_branch, {silent:true}).output;
			logstextarea(glogfilename, outputtext, 10);
			
			console.log("Deployment to " + description + " complete.");
			logstuff(glogfilename, "Deployment to " + description + " complete.");
			messageslackchannel("Deployment to <http://" + deployment_nginx_host_name + "|" + description + "> complete (requested by " + deployment_email + ") - <" + WEBSITE_LOGS_LOCATION + glogfilename + "|view log file>");
		}
	}		
}
...
//				var outputtext = "not running yet";
				...
				// update static html files - hacky shit
				if (sitename.toLowerCase().indexOf("prod") > -1) {
					// production site so key is probably ok
				} else {
					// non production site so need to update the google analytics keys to non prod
					console.log("updating static html files to contain correct google analytics key");
					logstuff(glogfilename, "Updating static html files to contain correct google analytics key");				
					replacetextinfile(glogfilename, deployment_node_website_root + '/terms.html', '3UEXfpVIFogW14HvHCxDMDgicUIZiB0L', 'F8LKY9dVE2OHbftx11ey038R9mzEOb1X');
					replacetextinfile(glogfilename, deployment_node_website_root + '/team.html', '3UEXfpVIFogW14HvHCxDMDgicUIZiB0L', 'F8LKY9dVE2OHbftx11ey038R9mzEOb1X');
					replacetextinfile(glogfilename, deployment_node_website_root + '/privacy.html', '3UEXfpVIFogW14HvHCxDMDgicUIZiB0L', 'F8LKY9dVE2OHbftx11ey038R9mzEOb1X');
					replacetextinfile(glogfilename, deployment_node_website_root + '/index.html', '3UEXfpVIFogW14HvHCxDMDgicUIZiB0L', 'F8LKY9dVE2OHbftx11ey038R9mzEOb1X');
				}
...
function replacetextinfile(glogfilename, txtfilename, txtorigtext, txtrepltext) {
	var fs = require('fs');
	console.log('Replacing text ' + txtorigtext + ' with text ' + txtrepltext + ' in file ' + txtfilename);
	logstuff(glogfilename, 'Replacing text ' + txtorigtext + ' with text ' + txtrepltext + ' in file ' + txtfilename);
	txtfiledata = fs.readFileSync(txtfilename).toString();
	txtfiledata = txtfiledata.replace(txtorigtext, txtrepltext);
	fs.writeFileSync(txtfilename, txtfiledata);
}
...
//			if ((sitename.toLowerCase().indexOf("_staging") > -1) || (sitename.toLowerCase().indexOf("_int") > -1)) {
				output = execSync(HELPERS_SCRIPTS_FOLDER_LOCATION + 'killpm2nodescript.sh ' + sitename);
//			} else {
...
//			}
			...
			// update /client/views/build/core.js for replylite - hacky shit
			if (sitename.toLowerCase().indexOf("replylite") > -1) {
				// replylite deployment so need to update api domain name for client
				if (sitename.toLowerCase().indexOf("prod") > -1) {
					console.log("updating /client/views/build/core.js decidz api key to decidz.com");
					logstuff(glogfilename, "updating /client/views/build/core.js decidz api key to decidz.com");
					replacetextinfile(glogfilename, node_website_root + '/client/views/build/core.js', 'http://api.int.decidz.com/', 'https://api.decidz.com/');
					replacetextinfile(glogfilename, node_website_root + '/client/views/build/core.js', 'http://int.decidz.com/', 'https://decidz.com/');
				}
				if (sitename.toLowerCase().indexOf("staging") > -1) {
					console.log("updating /client/views/build/core.js decidz api key to staging.decidz.com");
					logstuff(glogfilename, "updating /client/views/build/core.js decidz api key to staging.decidz.com");				
					replacetextinfile(glogfilename, node_website_root + '/client/views/build/core.js', 'http://api.int.decidz.com/', 'http://api.staging.decidz.com/');
					replacetextinfile(glogfilename, node_website_root + '/client/views/build/core.js', 'http://int.decidz.com/', 'http://staging.decidz.com/');
				}
			}
			...
			// check if staging and then use pm2
//			if ((sitename.toLowerCase().indexOf("_staging") > -1) || (sitename.toLowerCase().indexOf("_int") > -1)) {
				output = execSync(HELPERS_SCRIPTS_FOLDER_LOCATION + 'startpm2nodescript.sh ' + node_website_root + '/' + sitename + '_worker.js ' + sitename + ' ' + node_website_root);
//			} else {
